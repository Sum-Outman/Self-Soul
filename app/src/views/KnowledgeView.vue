    ...
    <div v-else-if="activeTab === 'browse'" class="content">
      <div class="browse-controls">
        <div class="search-box">
          ...
          <button @click="exportSearchResults" :disabled="searchResults.length === 0 || exportInProgress">
            {{ exportInProgress ? 'Exporting...' : 'Export Results' }}
          </button>
          <select v-model="exportFormat" style="margin-left: 8px;">
            <option value="json">JSON</option>
            <option value="csv">CSV</option>
            <option value="text">Text</option>
          </select>
          <button @click="loadKnowledgeGraph" :disabled="searchResults.length === 0 || graphLoading">
            {{ graphLoading ? 'Loading Graph...' : 'Show Knowledge Graph' }}
          </button>
        </div>
        ...
        <div v-if="showAdvancedSearch" class="advanced-search-panel">
          <div class="advanced-filters">
            <div class="filter-group">
              ...
              <input type="date" v-model="searchDateRange[0]">
              <span>to</span>
              <input type="date" v-model="searchDateRange[1]">
            </div>
            <div class="filter-group">
              <label>File Types:</label>
              <select v-model="searchFileTypes" multiple style="height: 80px;">
            ...
            <div class="filter-group">
              ...
              <input type="range" v-model="relevanceThreshold" min="0" max="1" step="0.1">
            </div>
            <div class="filter-group">
              <label>Sort By:</label>
              ...
              <select v-model="searchSortOrder">
                <option value="desc">Descending</option>
                <option value="asc">Ascending</option>
              </select>
            </div>
            <button @click="advancedSearchKnowledge">Apply Advanced Filters</button>
          </div>
        </div>
        
        <!-- Batch Operations Toolbar -->
        <div v-if="searchResults.length > 0" class="batch-operations-toolbar">
          <label><input type="checkbox" @change="selectAllFiles"> Select All</label>
          <span>Selected: {{ selectedFiles.length }}</span>
          <button @click="batchDeleteFiles" :disabled="selectedFiles.length === 0">Delete Selected</button>
          <button @click="applyAiFeature('summarize')" :disabled="selectedFiles.length === 0 || aiFeatureInProgress">Summarize</button>
          <button @click="applyAiFeature('translate')" :disabled="selectedFiles.length === 0 || aiFeatureInProgress">Translate</button>
          <button @click="applyAiFeature('extract_keywords')" :disabled="selectedFiles.length === 0 || aiFeatureInProgress">Extract Keywords</button>
        </div>
      </div>

      <!-- Knowledge Graph Visualization -->
      <div v-if="showKnowledgeGraph && graphData" class="knowledge-graph-container">
        <h3>Knowledge Graph</h3>
        <div v-if="graphData.nodes.length > 0" class="knowledge-graph-visualization">
          <div class="graph-stats">
            <p>Graph contains {{ graphData.nodes.length }} nodes and {{ graphData.edges.length }} edges.</p>
          </div>
          <div class="graph-nodes-edges">
            <div class="nodes-list">
              <h4>Nodes ({{ graphData.nodes.length }})</h4>
              <ul>
                <li v-for="node in graphData.nodes.slice(0, 10)" :key="node.id">
                  {{ node.label }} ({{ node.type }})
                </li>
                <li v-if="graphData.nodes.length > 10">... and {{ graphData.nodes.length - 10 }} more</li>
              </ul>
            </div>
            <div class="edges-list">
              <h4>Edges ({{ graphData.edges.length }})</h4>
              <ul>
                <li v-for="edge in graphData.edges.slice(0, 10)" :key="edge.source + '-' + edge.target">
                  {{ edge.source }} → {{ edge.target }} ({{ edge.label }})
                </li>
                <li v-if="graphData.edges.length > 10">... and {{ graphData.edges.length - 10 }} more</li>
              </ul>
            </div>
          </div>
        </div>
        <div v-else class="graph-placeholder">
          <p>No graph data available. Load knowledge graph to visualize relationships.</p>
        </div>
        <button @click="showKnowledgeGraph = false">Hide Graph</button>
      </div>
      ...
      <div v-if="searchResults.length > 0" class="search-results">
        ...
        <div v-for="(result, index) in searchResults" :key="index" class="result-item">
          <div class="result-selection">
            <input type="checkbox" :checked="selectedFiles.some(f => f.id === result.id)" @change="toggleFileSelection(result)">
          </div>
          <div class="result-content-wrapper">
            ...
            <div class="result-meta">
              ...
              <span class="result-date">{{ result.timestamp ? formatDate(result.timestamp) : '' }}</span>
            </div>
          </div>
        </div>
      </div>
...
export default {
  name: 'KnowledgeView',
  components: {
    // KnowledgeImport component not used, removed
  },
  setup() {
    ...
    const relevanceThreshold = ref(0.5);
    const searchSortBy = ref('relevance');
    ...
    // AI features
    const aiFeatureInProgress = ref(false);
    const selectedAiFeature = ref('');
    ...
    const searchKnowledge = async () => {
      ...
      return await performDataLoad('search-knowledge', {
        apiCall: () => api.knowledge.search(searchQuery.value, searchDomain.value),
        ...
        onError: (error) => {
          searchResults.value = [];
          searchPerformed.value = true;
          showSystemMessage('Knowledge search failed. Please ensure the backend service is running.');
        },
        successMessage: '', // We handle messages manually
        errorMessage: 'Knowledge search failed',
        errorContext: 'Search Knowledge',
        showSuccess: false,
        showError: false,
        notify: notify,
    ...
    const advancedSearchKnowledge = async () => {
      if (!searchQuery.value.trim()) return;
      ...
      try {
        ...
        if (response.data.success) {
          ...
          if (searchDateRange.value.length === 2) {
            ...
            results = results.filter(item => {
              const itemDate = new Date(item.timestamp || item.upload_date);
              return itemDate >= new Date(startDate) && itemDate <= new Date(endDate);
            });
          }
          
          if (searchFileTypes.value.length > 0) {
            results = results.filter(item => {
              const fileType = item.source?.split('.').pop()?.toLowerCase() || '';
              return searchFileTypes.value.includes(fileType);
            });
          }
          
          // Apply relevance threshold
          if (relevanceThreshold.value > 0) {
            results = results.filter(item => item.relevance >= relevanceThreshold.value);
          }
          
          // Sort results
          results.sort((a, b) => {
            let valueA, valueB;
            switch (searchSortBy.value) {
              case 'relevance':
                valueA = a.relevance || 0;
                valueB = b.relevance || 0;
                break;
              case 'date':
                valueA = new Date(a.timestamp || a.upload_date);
                valueB = new Date(b.timestamp || b.upload_date);
                break;
              case 'title':
            ...
            return searchSortOrder.value === 'asc' ? valueA - valueB : valueB - valueA;
          });
        ...
        handleApiError(error, 'Advanced Search');
        searchResults.value = [];
        searchPerformed.value = true;
        showSystemMessage('Advanced search failed. Please ensure the backend service is running.');
      }
    };
    ...
    const batchDeleteFiles = async () => {
      ...
      const confirmDelete = confirm(`Are you sure you want to delete ${selectedFiles.value.length} selected files?`);
      if (!confirmDelete) return;
      
      try {
        const deletePromises = selectedFiles.value.map(file => 
          api.knowledge.deleteFile(file.id).catch(err => {
        ...
        showSystemMessage(`Successfully deleted ${successCount}/${selectedFiles.value.length} files.`);
        ...
        handleApiError(error, 'Batch Delete Files');
        showSystemMessage('Failed to delete selected files.');
      }
    };
    ...
    const exportSearchResults = async () => {
      if (searchResults.value.length === 0) {
        showSystemMessage('No search results available for export.');
        return;
      }
      ...
      try {
        ...
        switch (exportFormat.value) {
          ...
          case 'csv':
            // Convert to CSV
            const headers = ['Title', 'Content', 'Relevance', 'Source', 'Timestamp'];
            ...
            break;
          case 'text':
            exportData = searchResults.value.map(item => 
        ...
        link.href = url;
        link.download = `knowledge_export_${timestamp}.${exportFormat.value}`;
        document.body.appendChild(link);
        link.click();
        document.body.removeChild(link);
        URL.revokeObjectURL(url);
        
        showSystemMessage(`Successfully exported ${searchResults.value.length} items.`);
      } catch (error) {
        handleApiError(error, 'Export Search Results');
        showSystemMessage('Export search results failed.');
      } finally {
    ...
    const loadKnowledgeGraph = async () => {
      ...
      try {
        // Fetch real knowledge graph data from API with timeout
        const timeoutPromise = new Promise((_, reject) => {
          setTimeout(() => reject(new Error('Knowledge graph API request timed out')), 10000); // 10 second timeout
        });
        ...
        if (response.data.success && response.data.graph) {
          ...
          showSystemMessage('Knowledge graph loaded successfully');
        } else {
          showSystemMessage('Knowledge graph API returned invalid data');
        }
      } catch (error) {
        handleApiError(error, 'Load Knowledge Graph');
        showSystemMessage('Knowledge graph loading failed');
      } finally {
        graphLoading.value = false;
      }
    };

    // Apply AI feature to selected files
    const applyAiFeature = async (feature) => {
      if (selectedFiles.value.length === 0) {
        showSystemMessage('Please select files to apply AI feature');
        return;
      }
      
      aiFeatureInProgress.value = true;
      selectedAiFeature.value = feature;
      
      try {
        // Prepare file IDs for API request
        const fileIds = selectedFiles.value.map(file => file.id);
        
        // Map feature names to API endpoints
        const featureEndpoints = {
          'summarize': '/api/knowledge/summarize',
          'translate': '/api/knowledge/translate',
          'extract_keywords': '/api/knowledge/extract-keywords'
        };
        ...
        // Call real API endpoint with timeout
        const timeoutPromise = new Promise((_, reject) => {
          setTimeout(() => reject(new Error(`AI feature ${feature} request timed out`)), 30000); // 30 second timeout, AI processing may take longer
        });
        ...
        const response = await Promise.race([
          api.post(endpoint, {
            ...
            parameters: {
              ...
              max_length: feature === 'summarize' ? 200 : undefined,
              target_language: feature === 'translate' ? 'en' : undefined
            }
          }),
          timeoutPromise
        ]);
        ...
        if (response.data.success) {
          ...
          showSystemMessage(`Successfully applied ${feature} to ${fileIds.length} files`);
          
          // Display results if available
          if (result.summary || result.keywords || result.translation) {
            if (import.meta.env.DEV) {
              console.log(`AI feature ${feature} result:`, result);
            }
            // In a real implementation, you would display the results in a modal or dedicated section
          }
        } else {
          throw new Error(response.data.message || `AI feature ${feature} failed`);
        }
      } catch (error) {
        console.error(`AI feature ${feature} failed:`, error);
        handleApiError(error, `AI Feature: ${feature}`);
        showSystemMessage(`Applying ${feature} failed: ${error.message}`);
      } finally {
        aiFeatureInProgress.value = false;
        selectedAiFeature.value = '';
      }
    };
    ...
    const loadKnowledgeStats = async () => {
      ...
      try {
        ...
        if (error.code === 'ECONNREFUSED') {
          ...
          handleApiError(error, 'Load Statistics Information');
        }
    ...
    const loadFiles = async () => {
      ...
      try {
        ...
        if (error.code === 'ECONNREFUSED') {
          ...
          handleApiError(error, 'Load File List');
        }
        files.value = [];
        filterFiles();
    ...
    const viewFile = async (file) => {
      try {
        ...
        if (!newTab || newTab.closed || typeof newTab.closed === 'undefined') {
          showSystemMessage('Please allow pop-ups to view files');
    ...
    const copyText = async () => {
      try {
        ...
        handleApiError(error, 'Failed to copy text');
    ...
    const downloadFile = async (file) => {
      try {
        ...
        // Use apiClient for blob response type with proper proxy configuration
        const response = await apiClient.get(`/api/knowledge/files/${file.id}/download`, {
          responseType: 'blob',
        ...
        showSystemMessage(`Cannot download file: ${file.name}. Please ensure backend service is running.`);
      }
    };
    ...
    const connectWebSocket = (sessionId) => {
      try {
        ...
        addLearningLog('System', 'Skipping WebSocket connection, using polling mode for more reliable updates');
        showSystemMessage('Using polling mode for progress updates');
        
        // Start polling immediately
        startProgressPolling();
      } catch (error) {
        handleApiError(error, 'Auto learning connection error');
    ...
    const handleWebSocketError = (error) => {
      ...
      if (autoLearningStatus.value === 'running' && !isUsingWebSocket) {
        addLearningLog('System', `WebSocket connection failed: ${errorMessage}. Using polling mode instead.`);
        startProgressPolling();
      }
    };
    ...
    const completeAutoLearning = () => {
      ...
      // Refresh files to show any new knowledge with retry logic
      const refreshFilesWithRetry = async (retryCount = 0) => {
        try {
          await loadFiles();
          ...
          if (retryCount < 3) {
            if (import.meta.env.DEV) {
              console.log(`File refresh failed, ${retryCount + 1}th retry...`);
            }
            setTimeout(() => { if (isMounted.value) refreshFilesWithRetry(retryCount + 1); }, 2000);
          } else {
            console.error('File refresh failed, maximum retry count reached:', error);
            showSystemMessage('File refresh failed, please refresh manually');
          }
        }
      };
      
      // Start file refresh after a short delay to allow server processing
      setTimeout(() => { if (isMounted.value) refreshFilesWithRetry(); }, 1000);
    };
    ...
    const startAutoLearning = async () => {
      try {
        ...
        if (response.data.status === 'success' || response.data.status === 'warning') {
          showSystemMessage('Auto-learning successfully started');
        ...
        handleApiError(error, 'Auto learning start failed');
    ...
    const stopAutoLearning = async () => {
      try {
        ...
        try {
          ...
          if (response.data.status === 'success' || response.data.status === 'warning') {
            showSystemMessage('Auto-learning successfully stopped');
        ...
        handleApiError(error, 'Stop learning error');
      }
    };
    ...
    const startProgressPolling = () => {
      ...
      addLearningLog('System', `Starting polling mode (interval: ${pollingInterval/1000} seconds)`);
      ...
      progressPollingInterval = setInterval(async () => {
        try {
          ...
          // Create a timeout promise for the progress API call
          const timeoutPromise = new Promise((_, reject) => {
            setTimeout(() => reject(new Error('Progress query timeout')), 10000); // 10 second timeout
          });
          
          // Race between API call and timeout
          const response = await Promise.race([
            api.knowledge.autoLearning.progress(),
            timeoutPromise
          ]);
          ...
          if (consecutiveFailures <= maxFailures) {
            ...
            addLearningLog('System', `Maximum polling failure count reached (${maxFailures})`);
            showSystemMessage('Cannot communicate with server, connection failed');
    ...
    // File upload handling with enhanced error handling and concurrency control
    const handleFileUpload = async (event) => {
      ...
      for (let i = 0; i < files.length; i++) {
        ...
        // Enhanced file type validation with fallback to file extension
        let isFileTypeValid = false;
        
        // 1. Check by MIME type
        if (file.type && allowedTypes.includes(file.type)) {
          isFileTypeValid = true;
        } else {
          // 2. Fallback to file extension check
          const allowedExtensions = ['.pdf', '.doc', '.docx', '.ppt', '.pptx', '.txt', '.json', '.jpg', '.jpeg', '.png'];
          const fileExtension = '.' + file.name.toLowerCase().split('.').pop();
          
          if (allowedExtensions.includes(fileExtension)) {
            isFileTypeValid = true;
            if (import.meta.env.DEV) {
              console.log(`File type validation fallback: ${file.name} validated by extension ${fileExtension}`);
            }
          }
        }
        
        if (!isFileTypeValid) {
          showSystemMessage(`File type not supported: ${file.name}. Supported file types: PDF, DOC/DOCX, PPT/PPTX, TXT, JSON, JPG, PNG`);
          continue;
        }
        ...
        try {
          ...
          if (selectedDomain.value === 'autoDetect') {
            ...
            showSystemMessage(`Auto-detecting domain for ${file.name}...`);
          } else {
          ...
          const uploadTimeoutPromise = new Promise((_, reject) => {
            setTimeout(() => reject(new Error(`File upload timeout: ${file.name}`)), 30000); // 30 second timeout
          });
          
          // Upload file with progress tracking and timeout
          const uploadPromise = api.post('/api/knowledge/upload', formData, {
            timeout: 30000,
          ...
          handleApiError(error, 'Upload Error');
          
          if (error.message.includes('timeout')) {
            showSystemMessage(`File upload timeout: ${file.name}. Please check network connection or server status.`);
          } else if (error.code === 'ECONNREFUSED') {
            showSystemMessage(`Cannot connect to backend service. Please ensure backend service is running: ${file.name}`);
          } else {
            showSystemMessage(`File upload failed: ${file.name}. Error: ${error.message}`);
          }
        } finally {
    ...
    return {
      ...
      loadFiles,
      filterFiles,
      sortFiles, // Add missing sortFiles function
      nextPage,
      prevPage,
      viewFile,
      downloadFile,
...
.batch-operations-toolbar label {
  display: flex;
  align-items: center;
  gap: var(--spacing-sm);
  cursor: pointer;
...
.batch-operations-toolbar button {
  padding: var(--spacing-sm) var(--spacing-md);
  border: 1px solid var(--border-light);
  background: var(--bg-primary);
  border-radius: var(--border-radius);
...
.batch-operations-toolbar button:hover:not(:disabled) {
  background: var(--bg-tertiary);
  border-color: var(--accent-color);
}

.batch-operations-toolbar button:disabled {
  opacity: 0.5;
  cursor: not-allowed;
}

/* Knowledge Graph Container Styles */
...
/* Search Result Selection Styles */
.result-item {
  display: flex;
  align-items: flex-start;
  gap: var(--spacing-md);
