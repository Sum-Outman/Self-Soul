...
learning_sessions: Dict[str, List[bool]] = {}

async def generate_learning_progress(
    session_id: str, 
    websocket: Any, 
    is_active: List[bool]
) -> None:
    """
    生成逼真的学习进度更新并通过WebSocket发送
    
    Args:
        session_id: 学习会话的唯一标识符
        websocket: WebSocket连接对象
        is_active: 控制进度生成的标志列表（第一个元素为True时继续生成）
    ...
    for phase in phases:
        ...
        for _ in range(phase_steps):
            ...
            if log_count % 4 == 0 and phase["name"] in phase_logs and phase_logs[phase["name"]]:
                log_message = phase_logs[phase["name"]][hash(session_id + phase["name"] + str(log_count)) % len(phase_logs[phase["name"]])]
...
async def send_message(websocket: Any, data: Dict[str, Any]) -> None:
    """
    发送WebSocket消息
...
async def handle_auto_learning(websocket: Any, path: str) -> None:
    """
    处理自动学习的WebSocket连接
