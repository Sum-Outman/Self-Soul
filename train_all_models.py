...
def main():
    ...
    try:
        ...
        # Start 2 cycles of training
        total_cycles = 2
        print(f"Starting {total_cycles} cycles of from-scratch training for all models...")
        
        for cycle in range(1, total_cycles + 1):
            print(f"\n{'='*60}")
            print(f"Cycle {cycle}/{total_cycles}: Starting training...")
            print(f"{'='*60}")
            
            cycle_start_time = time.time()
            
            # Reload all models before each cycle
            print(f"Loading all models for cycle {cycle}...")
            loaded_models = model_registry.load_all_models()
            print(f"Successfully loaded {len(loaded_models)} models for cycle {cycle}")
            
            # Start training all models in this cycle
            ...
            if result["success"]:
                ...
                print(f"\nCycle {cycle} completed in {cycle_duration:.2f} seconds")
            else:
                print(f"\nCycle {cycle} failed: {result.get('message', 'Unknown error')}")
                # Continue with next cycle even if this one failed
        
        print(f"\n{'='*80}")
        print(f"All {total_cycles} training cycles completed!")
        print(f"{'='*80}")
