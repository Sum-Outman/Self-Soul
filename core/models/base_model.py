...
try:
    ...
    logging.warning(
        "ExternalAPIService not available, models will use local processing only"
    )
...
try:
    ...
    logging.warning("EvolutionModule not available, evolution features will be limited")
...
class BaseModel(ABC):
    ...
    def __init__(self, config: Dict[str, Any] = None):
        ...
        if EXTERNAL_API_AVAILABLE:
            try:
                self.external_api_service = ExternalAPIService(
                    config.get("external_api_config", {})
                )
                self.logger.info(
                    f"External API service initialized for {self.model_id}"
                )
            except Exception as e:
                error_handler.log_warning(
                    f"Failed to initialize external API service: {str(e)}", "BaseModel"
                )
        ...
        self.logger.info(
            f"Enhanced base model initialized: {self.model_id}, from_scratch: {self.from_scratch}"
        )
    ...
    def set_mode(self, mode: str, config: Dict[str, Any] = None) -> Dict[str, Any]:
        ...
        if mode == "external":
            ...
            if not test_result["success"]:
                ...
                return {
                    "success": False,
                    "error": f"API连接测试失败: {test_result.get('error', 'Unknown error')}",
                }
    ...
    def train(
        self, training_data: Any, config: Dict[str, Any] = None
    ) -> Dict[str, Any]:
        """训练模型 | Train model"""
    ...
    def _train_from_scratch(
        ...
        try:
            ...
            # 准备数据（具体模型应覆盖此部分）
            train_loader, val_loader = self._prepare_training_data(
            ...
            if optimizer_type == "adam":
                self.optimizer = optim.Adam(
                    self.model_architecture.parameters(), lr=learning_rate
                )
            ...
            if val_loss < best_val_loss:
                ...
                torch.save(
                    {
                        "model_state_dict": self.model_architecture.state_dict(),
                    ...
                    os.path.join("models", f"{self.model_id}_best.pt"),
                )

                self.logger.info(
                    f"Epoch {epoch+1}/{epochs} - Loss: {train_loss:.4f}, Acc: {train_acc:.2f}% - Val Loss: {val_loss:.4f}, Val Acc: {val_acc:.2f}%"
                )
            ...
            torch.save(
                ...
                os.path.join("models", f"{self.model_id}_final.pt"),
            )
            ...
            self.is_training = False
            self.logger.error(
                f"从零开始训练失败: {str(e)} | From scratch training failed: {str(e)}"
            )
            return {"success": False, "error": str(e)}

    def _prepare_training_data(
        self, training_data: Any, batch_size: int, validation_split: float
    ) -> tuple:
        """准备训练数据，具体模型应覆盖此方法"""
    ...
    def _initialize_model_architecture(self, config: Dict[str, Any]):
        ...
        self.logger.info(
            f"模型架构初始化完成: {self.model_id}, 输入维度: {input_size}, 隐藏层: {hidden_sizes}, 输出维度: {output_size}"
        )
    ...
    def test_connection(self) -> Dict[str, Any]:
        ...
        try:
            ...
            actual_latency = 0.0
            error_message = ""
            http_client_used = "unknown"  # 跟踪使用的HTTP客户端库

            try:
                # 尝试导入requests库进行真实HTTP测试
                try:
                    import requests
                    from requests.exceptions import RequestException, Timeout

                    http_client_used = "requests"
                    ...
                    try:
                        response = requests.head(
                            api_url, headers=headers, timeout=timeout_seconds
                        )
                        end_time = time.time()
                        ...
                        if response.status_code < 400:
                            ...
                            http_client_used = "requests"
                            self.logger.info(
                                f"API连接测试成功: HTTP {response.status_code}, 延迟 {actual_latency:.3f}秒"
                            )
                        else:
                            error_message = f"HTTP错误 {response.status_code}"
                            self.logger.warning(f"API连接测试失败: {error_message}")

                    except Timeout:
                        error_message = f"连接超时 ({timeout_seconds}秒)"
                        self.logger.warning(f"API连接测试超时: {api_url}")
                    except RequestException as req_err:
                        error_message = f"请求错误: {str(req_err)}"
                        self.logger.warning(f"API连接测试请求错误: {req_err}")

                except ImportError:
                    # requests库不可用，尝试使用urllib进行真实HTTP测试
                    self.logger.warning("requests库不可用，尝试使用urllib进行HTTP测试")
                    try:
                        import urllib.request
                        import urllib.error
                        from urllib.parse import urlparse

                        http_client_used = "urllib"

                        # 解析URL以获取主机和路径
                        parsed_url = urlparse(api_url)
                        if not parsed_url.scheme or not parsed_url.netloc:
                            raise ValueError(f"无效的URL格式: {api_url}")
                        ...
                        # 发送请求
                        start_urllib_time = time.time()
                        try:
                            with urllib.request.urlopen(
                                req, timeout=timeout_seconds
                            ) as response:
                                end_time = time.time()
                                actual_latency = end_time - start_urllib_time
                                connection_success = response.status < 400
                                if connection_success:
                                    http_client_used = "urllib"
                                self.logger.info(
                                    f"urllib API连接测试成功: HTTP {response.status}, 延迟 {actual_latency:.3f}秒"
                                )
                        except urllib.error.HTTPError as http_err:
                            end_time = time.time()
                            actual_latency = end_time - start_urllib_time
                            error_message = f"HTTP错误 {http_err.code}: {http_err.reason}"
                            self.logger.warning(f"urllib API连接测试失败: {error_message}")
                        except urllib.error.URLError as url_err:
                            end_time = time.time()
                            actual_latency = end_time - start_urllib_time
                            error_message = f"URL错误: {str(url_err)}"
                            self.logger.warning(f"urllib API连接测试URL错误: {url_err}")

                    except ImportError:
                        # urllib也不可用，尝试使用http.client
                        self.logger.warning("urllib也不可用，尝试使用http.client进行HTTP测试")
                        try:
                            import http.client
                            import ssl
                            from urllib.parse import urlparse

                            http_client_used = "http.client"

                            parsed_url = urlparse(api_url)
                            if parsed_url.scheme == "https":
                                conn = http.client.HTTPSConnection(
                                    parsed_url.netloc, timeout=timeout_seconds
                                )
                            else:
                            ...
                            try:
                                ...
                                end_time = time.time()
                                actual_latency = end_time - start_http_time
                                connection_success = response.status < 400
                                if connection_success:
                                    http_client_used = "http.client"
                                self.logger.info(
                                    f"http.client API连接测试成功: HTTP {response.status}, 延迟 {actual_latency:.3f}秒"
                                )
                                conn.close()
                            except Exception as conn_err:
                                end_time = time.time()
                                actual_latency = end_time - start_http_time
                                error_message = f"http.client连接错误: {str(conn_err)}"
                                self.logger.warning(
                                    f"http.client API连接测试失败: {conn_err}"
                                )
                                conn.close()
                            ...
                            # 没有可用的HTTP客户端库
                            self.logger.error(
                                "没有可用的HTTP客户端库 (requests, urllib, http.client)"
                            )
                            error_message = "没有可用的HTTP客户端库，无法进行真实连接测试"
                            connection_success = False

                    except Exception as urllib_error:
                        error_message = f"urllib测试异常: {str(urllib_error)}"
                        self.logger.error(f"urllib测试异常: {urllib_error}")

            except Exception as test_error:
                error_message = f"连接测试异常: {str(test_error)}"
                self.logger.error(f"API连接测试异常: {test_error}")
            ...
            if connection_success:
                self.logger.info(
                    f"外部API连接测试成功: {self.model_id}, 延迟 {actual_latency:.3f}秒"
                )
                return {
                    "success": True,
                    "model_id": self.model_id,
                    ...
                    "test_method": f"real_http_test_{http_client_used}",
                    "message": f"API连接测试成功 | API connection test successful (延迟: {actual_latency:.3f}秒)",
                }
            else:
                self.logger.warning(
                    f"外部API连接测试失败: {self.model_id}, 错误: {error_message}"
                )
                return {
                    "success": False,
                    "error": error_message or "未知连接错误",
                    "model_id": self.model_id,
                    "api_url": api_url,
                    "timestamp": time.time(),
                    "latency_seconds": actual_latency,
                    "test_method": f"real_http_test_{http_client_used}",
                    "message": f"API连接测试失败 | API connection test failed: {error_message}",
                }
        except Exception as e:
            error_message = str(e)
    ...
    def get_api_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            else {"success": True, "message": "使用本地模式 | Using local mode"},
        }
    ...
    def _initialize_data_processors(self):
        ...
        for modality in self.supported_modalities:
            if modality == "text":
                ...
                error_handler.log_warning(
                    f"Unsupported modality: {modality}", "BaseModel"
                )
        ...
        try:
            ...
            for modality, data in input_data.items():
                if modality in self.data_processors:
                    ...
                    error_handler.log_warning(
                        f"No processor for modality: {modality}", "BaseModel"
                    )
                    results[modality] = {
                        "type": modality,
                        "data": data,
            ...
            self.is_processing = False
            return {"success": False, "error": str(e), "model_id": self.model_id}
    ...
    def _attempt_recovery(self, error: Exception, context: str):
        ...
        while recovery_attempts < max_attempts:
            try:
                ...
                self.logger.info(
                    f"Recovery successful after {recovery_attempts} attempts"
                )
                return

            except Exception as recovery_error:
                error_handler.log_warning(
                    f"Recovery attempt {recovery_attempts} failed: {str(recovery_error)}",
                    "BaseModel",
                )
    ...
    def _get_resource_usage(self) -> Dict[str, Any]:
        ...
        try:
            ...
            error_handler.log_warning(
                "psutil not available for resource monitoring", "BaseModel"
            )
    ...
    def optimize_performance(self, config: Dict[str, Any] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            # Memory optimization
            if self.performance_metrics.get("peak_memory_usage", 0) > config.get(
            ...
            # CPU optimization
            if self.performance_metrics.get("cpu_utilization", 0) > config.get(
                "cpu_threshold", 80
            ):
    ...
    # === 自主演化方法 (Autonomous Evolution Methods) ===
    ...
    def evolve_architecture(
        ...
        Args:
            performance_targets: 性能目标字典 (如 {"accuracy": 0.9, "efficiency": 0.8})
            constraints: 约束条件 (如 {"memory_mb": 500, "compute_gflops": 2.0})
        ...
        try:
            ...
            if not EVOLUTION_MODULE_AVAILABLE:
                return {
                    "success": False,
                    "error": "演化模块不可用",
                    "suggestion": "请检查core.evolution_module是否正确导入",
                }

            # 获取模型当前架构
            base_architecture = self.get_model_architecture()
            if not base_architecture:
                base_architecture = {
                    "type": self.model_id,
                    "layers": getattr(self, "layers", []),
                    "parameters": getattr(self, "parameters_count", 0),
                    "description": f"Base architecture for {self.model_id}",
                }
            ...
            # 执行架构演化
            evolution_result = evolution_module.evolve_architecture(
            ...
            # 如果演化成功，尝试应用新架构
            if evolution_result.success and evolution_result.evolved_architecture:
                apply_result = self._apply_evolved_architecture(
                    evolution_result.evolved_architecture
                )
                evolution_result_dict = {
                    "success": True,
                    "evolution_result": evolution_result,
                    "architecture_applied": apply_result.get("success", False),
                    "applied_architecture": apply_result.get("architecture", {}),
                    "performance_improvement": evolution_result.performance_metrics.get(
                        "overall_score", 0.0
                    ),
                }
                return evolution_result_dict

            # 如果演化失败，返回失败信息
            return {
                "success": False,
                "error": evolution_result.error_message or "架构演化失败",
                "evolution_result": evolution_result,
            }

        except Exception as e:
            self._handle_error(e, "architecture_evolution")
            return {"success": False, "error": str(e), "suggestion": "检查演化模块配置和模型架构信息"}

    def _apply_evolved_architecture(
        self, evolved_architecture: Dict[str, Any]
    ) -> Dict[str, Any]:
        """应用演化后的架构

        Args:
            evolved_architecture: 演化后的架构
        ...
        try:
            ...
            architecture_snapshot = {
                "model_id": self.model_id,
                "architecture": evolved_architecture,
                "timestamp": time.time(),
                "applied": False,  # 标记为未实际应用
            }

            # 将架构快照保存到模型状态中
            if not hasattr(self, "architecture_snapshots"):
                self.architecture_snapshots = []

            self.architecture_snapshots.append(architecture_snapshot)

            # 限制快照数量
            max_snapshots = 10
            if len(self.architecture_snapshots) > max_snapshots:
                self.architecture_snapshots = self.architecture_snapshots[
                    -max_snapshots:
                ]

            self.logger.info(
                f"架构快照已保存 (ID: {evolved_architecture.get('architecture_id', 'unknown')})"
            )
            ...
            return {
                ...
                "architecture": evolved_architecture,
                "snapshot_saved": True,
                "message": "架构已记录，需要手动或自动应用",
            }

        except Exception as e:
            self._handle_error(e, "apply_evolved_architecture")
            return {"success": False, "error": str(e)}

    def adaptive_learning(self, learning_data: Dict[str, Any] = None) -> Dict[str, Any]:
        """自适应学习 - 根据学习数据调整模型参数

        Args:
            learning_data: 学习数据，包含性能指标等
        ...
        try:
            ...
            if "performance" in learning_data:
                performance_metrics.update(learning_data["performance"])

            # 自适应调整学习率
            learning_rate_adjustment = self._adaptive_learning_rate_adjustment(
                performance_metrics
            )

            # 自适应调整批处理大小
            batch_size_adjustment = self._adaptive_batch_size_adjustment(
                performance_metrics
            )

            # 自适应调整其他参数
            parameter_adjustments = self._adaptive_parameter_adjustment(
                performance_metrics
            )

            adjustments = {
                "learning_rate": learning_rate_adjustment,
                "batch_size": batch_size_adjustment,
                **parameter_adjustments,
            }

            # 应用调整（在实际系统中，这里应该更新模型参数）
            applied_count = 0
            for param_name, adjustment in adjustments.items():
                if adjustment.get("adjustment_applied", False):
                    applied_count += 1
            ...
            self.logger.info(f"自适应学习完成: {applied_count} 个参数已调整")
            return result

        except Exception as e:
            self._handle_error(e, "adaptive_learning")
            return {"success": False, "error": str(e)}

    def _adaptive_learning_rate_adjustment(
        self, performance_metrics: Dict[str, Any]
    ) -> Dict[str, Any]:
        """自适应学习率调整"""
        try:
            # 获取当前学习率
            current_lr = getattr(self, "learning_rate", 0.001)
            ...
            if accuracy > 0.8 and loss < 0.3:
                # 性能良好，降低学习率进行微调
                new_lr = current_lr * 0.9
                adjustment_reason = "高性能，降低学习率进行微调"
            elif accuracy < 0.5 or loss > 1.0:
                # 性能较差，增加学习率
                new_lr = current_lr * 1.1
                adjustment_reason = "低性能，增加学习率"
            else:
                # 性能中等，保持学习率
                adjustment_reason = "性能中等，保持学习率"
            ...
            return {
                ...
                "adjustment_applied": False,  # 标记为未实际应用
                "adjustment_reason": adjustment_reason,
                "timestamp": time.time(),
            }

        except Exception as e:
            self.logger.error(f"学习率调整失败: {str(e)}")
            return {
                "current_learning_rate": getattr(self, "learning_rate", 0.001),
                "new_learning_rate": getattr(self, "learning_rate", 0.001),
                "adjustment_applied": False,
                "adjustment_reason": f"调整失败: {str(e)}",
                "timestamp": time.time(),
            }

    def _adaptive_batch_size_adjustment(
        self, performance_metrics: Dict[str, Any]
    ) -> Dict[str, Any]:
        """自适应批处理大小调整"""
        try:
            ...
            if memory_usage > memory_threshold * 0.8:
                # 内存使用率高，减少批处理大小
                new_batch_size = max(1, current_batch_size // 2)
                adjustment_reason = f"高内存使用 ({memory_usage} bytes)，减少批处理大小"
            elif memory_usage < memory_threshold * 0.3 and current_batch_size < 256:
                # 内存使用率低，增加批处理大小
                new_batch_size = current_batch_size * 2
                adjustment_reason = f"低内存使用 ({memory_usage} bytes)，增加批处理大小"
            else:
                adjustment_reason = "内存使用适中，保持批处理大小"
            ...
            self.logger.error(f"批处理大小调整失败: {str(e)}")
            return {
                "current_batch_size": getattr(self, "batch_size", 32),
                "new_batch_size": getattr(self, "batch_size", 32),
                "adjustment_applied": False,
                "adjustment_reason": f"调整失败: {str(e)}",
                "timestamp": time.time(),
            }

    def _adaptive_parameter_adjustment(
        self, performance_metrics: Dict[str, Any]
    ) -> Dict[str, Any]:
        """自适应其他参数调整"""
        adjustments = {}
        ...
        # 调整正则化参数
        adjustments["regularization"] = self._adjust_regularization(performance_metrics)

        # 调整dropout率
        adjustments["dropout"] = self._adjust_dropout(performance_metrics)

        # 调整优化器参数
        adjustments["optimizer"] = self._adjust_optimizer(performance_metrics)
    ...
    def _adjust_regularization(
        ...
        """调整正则化参数"""
        try:
            current_reg = getattr(self, "regularization", 0.01)
            overfitting = performance_metrics.get("overfitting_score", 0.0)
            ...
            if overfitting > 0.7:
                ...
                adjustment_reason = f"过拟合风险高 ({overfitting:.2f})，增加正则化"
            elif overfitting < 0.3:
                new_reg = max(current_reg * 0.8, 0.001)
                adjustment_reason = f"过拟合风险低 ({overfitting:.2f})，减少正则化"
            else:
                adjustment_reason = "过拟合风险适中，保持正则化"

            return {
                "current_regularization": current_reg,
                "new_regularization": new_reg,
                "adjustment_applied": False,
                "adjustment_reason": adjustment_reason,
            }

        except Exception as e:
            self.logger.error(f"正则化调整失败: {str(e)}")
            return {
                "current_regularization": getattr(self, "regularization", 0.01),
                "new_regularization": getattr(self, "regularization", 0.01),
                "adjustment_applied": False,
                "adjustment_reason": f"调整失败: {str(e)}",
            }

    def _adjust_dropout(self, performance_metrics: Dict[str, Any]) -> Dict[str, Any]:
        """调整dropout率"""
        try:
            current_dropout = getattr(self, "dropout_rate", 0.5)
            generalization = performance_metrics.get("generalization_score", 0.5)

            new_dropout = current_dropout
            adjustment_reason = ""

            if generalization < 0.4:
                new_dropout = min(current_dropout * 1.2, 0.7)
                adjustment_reason = f"泛化能力差 ({generalization:.2f})，增加dropout"
            elif generalization > 0.6:
                new_dropout = max(current_dropout * 0.8, 0.3)
                adjustment_reason = f"泛化能力强 ({generalization:.2f})，减少dropout"
            else:
                adjustment_reason = "泛化能力适中，保持dropout"
            ...
            self.logger.error(f"Dropout调整失败: {str(e)}")
            return {
                "current_dropout": getattr(self, "dropout_rate", 0.5),
                "new_dropout": getattr(self, "dropout_rate", 0.5),
                "adjustment_applied": False,
                "adjustment_reason": f"调整失败: {str(e)}",
            }

    def _adjust_optimizer(self, performance_metrics: Dict[str, Any]) -> Dict[str, Any]:
        """调整优化器参数"""
        return {
            "current_optimizer": getattr(self, "optimizer_type", "Adam"),
            "new_optimizer": getattr(self, "optimizer_type", "Adam"),
            "adjustment_applied": False,
            "adjustment_reason": "优化器参数调整暂未实现",
        }
    ...
    def get_model_architecture(self) -> Optional[Dict[str, Any]]:
        ...
        try:
            # 尝试获取模型架构信息
            if hasattr(self, "get_architecture"):
            ...
            architecture = {
                ...
                "parameters_count": getattr(self, "parameters_count", 0),
                "layers": getattr(self, "layers", []),
                "activation_functions": getattr(self, "activation_functions", []),
                "optimizer": getattr(self, "optimizer_type", "unknown"),
            ...
            self.logger.error(f"获取模型架构失败: {str(e)}")
            return None
    ...
    def get_evolution_status(self) -> Dict[str, Any]:
        ...
        try:
            # 检查演化模块是否可用
            if EVOLUTION_MODULE_AVAILABLE:
                evolution_module = get_evolution_module()
                module_status = evolution_module.get_evolution_status()
                statistics = evolution_module.get_evolution_statistics()
            else:
                module_status = {"error": "演化模块不可用"}
                statistics = {}

            # 获取模型特定的演化信息
            model_evolution_info = {
                "model_id": self.model_id,
                "architecture_snapshots_count": len(
                    getattr(self, "architecture_snapshots", [])
                ),
                "last_evolution_time": None,
                "evolution_enabled": EVOLUTION_MODULE_AVAILABLE,
            }

            # 如果有架构快照，获取最后更新时间
            if hasattr(self, "architecture_snapshots") and self.architecture_snapshots:
                model_evolution_info[
                    "last_evolution_time"
                ] = self.architecture_snapshots[-1].get("timestamp")

            return {
                "model_evolution": model_evolution_info,
                "module_status": module_status,
                "statistics": statistics,
                "evolution_module_available": EVOLUTION_MODULE_AVAILABLE,
                "timestamp": time.time(),
            }

        except Exception as e:
            self._handle_error(e, "get_evolution_status")
            return {
                "success": False,
                "error": str(e),
                "evolution_module_available": EVOLUTION_MODULE_AVAILABLE,
    ...
    def use_external_api_service(
        self, api_type: str, service_type: str, data: Any
    ) -> Dict[str, Any]:
        """使用统一的外部API服务处理数据"""
    ...
    def process_enhanced(
        ...
        try:
            ...
            if use_external_api and self.external_api_service:
                # 使用外部API服务
                api_result = self.use_external_api_service(
                    api_type, "image", input_data
                )
                if api_result["success"]:
                    result = api_result["result"]
                else:
                    error_handler.log_warning(
                        f"External API failed, falling back to local processing: {api_result.get('error')}",
                        "BaseModel",
                    )
