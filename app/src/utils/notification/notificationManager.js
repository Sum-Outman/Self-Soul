...
class NotificationManager {
  ...
  displayNotification(notification) {
    ...
    // Create content container safely using DOM methods
    const contentEl = document.createElement('div');
    contentEl.className = 'notification-content';
    
    const messageSpan = document.createElement('span');
    messageSpan.className = 'notification-message';
    messageSpan.textContent = notification.message; // 使用textContent避免XSS
    ...
    contentEl.appendChild(messageSpan);
    contentEl.appendChild(closeBtn);
    notificationEl.appendChild(contentEl);

    // Add close button event listener (use the already created closeBtn variable)
    if (closeBtn) {
