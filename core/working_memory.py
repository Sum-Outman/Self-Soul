...
try:
    ...
    logging.warning(f"无法导入多模态处理器: {e}")
...
class WorkingMemory:
    ...
    def __init__(self, capacity: int = 7, decay_factor: float = 0.99, activation_threshold: float = 0.1):
        ...
        if MULTIMODAL_PROCESSOR_AVAILABLE and MultimodalProcessor is not None:
            try:
        ...
        # 初始化基本文本嵌入生成器
        self.text_embedding_generator = BasicTextEmbeddingGenerator()
        logger.info("基本文本嵌入生成器已初始化")
    ...
    def add_memory(self, content: Any, modality: str, embedding: Optional[np.ndarray] = None,
        ...
        if embedding is None:
            if modality == 'text' and self.text_embedding_generator is not None:
                # 使用基本文本嵌入生成器
                try:
                    text_content = str(content)
                    embedding = self.text_embedding_generator.generate_embedding(text_content)
                    # 对于文本内容，根据内容长度和复杂性设置基本置信度
                    confidence = min(0.8, 0.3 + len(text_content.split()) * 0.02)
                except Exception as e:
                    logger.debug(f"使用文本嵌入生成器失败: {e}")
            elif self.multimodal_processor is not None:
                # 尝试使用多模态处理器生成
                try:
    ...
    def _get_embedding(self, content: Any, modality: str = None) -> Optional[np.ndarray]:
        ...
        if modality == 'text':
            # 使用基本文本嵌入生成器
            if self.text_embedding_generator is not None:
                try:
                    text_content = str(content)
                    return self.text_embedding_generator.generate_embedding(text_content)
                except Exception as e:
                    logger.debug(f"文本嵌入生成器获取嵌入失败: {e}")
        ...
        # 如果无法获取嵌入，生成基本的随机嵌入作为最后手段
        # 这比返回None更好，因为至少允许记忆系统继续工作
        logger.debug(f"无法为内容生成嵌入，使用基本随机嵌入: modality={modality}")
        return self._generate_basic_embedding(content, modality)
    ...
    def get_current_context(self) -> Dict[str, Any]:
        ...
        返回:
            包含当前工作记忆状态的字典，包括统计信息、最近记忆、高激活记忆等
        """
        import time
        ...
        try:
            ...
            sorted_by_time = sorted(all_memories, key=lambda x: x.timestamp, reverse=True)[:3]
            
            for mem_item in sorted_by_time:
                recent_memories.append({
                    'memory_id': mem_item.memory_id,
                    'modality': mem_item.modality,
                    'content_preview': str(mem_item.content)[:50] + ('...' if len(str(mem_item.content)) > 50 else ''),
                    'confidence': mem_item.confidence,
                    'activation_level': mem_item.activation_level,
                    'timestamp': mem_item.timestamp.isoformat() if hasattr(mem_item.timestamp, 'isoformat') else str(mem_item.timestamp)
                })
            
            # 获取激活水平最高的记忆
            high_activation_memories = []
            sorted_by_activation = sorted(all_memories, key=lambda x: x.activation_level, reverse=True)[:3]
            
            for mem_item in sorted_by_activation:
                high_activation_memories.append({
                    'memory_id': mem_item.memory_id,
                    'modality': mem_item.modality,
                    'activation_level': mem_item.activation_level,
                    'access_count': mem_item.access_count,
                    'last_accessed': mem_item.last_accessed.isoformat() if hasattr(mem_item.last_accessed, 'isoformat') else str(mem_item.last_accessed)
                })
            
            # 获取各个组件的状态
            component_status = {
                'central_executive': {
                    'count': len(self.central_executive),
                    'status': 'active' if len(self.central_executive) > 0 else 'inactive'
                },
                'phonological_loop': {
                    'count': len(self.phonological_loop),
                    'capacity': self.capacity,
                    'utilization': len(self.phonological_loop) / self.capacity if self.capacity > 0 else 0
                },
                'visuospatial_sketchpad': {
                    'count': len(self.visuospatial_sketchpad),
                    'capacity': self.capacity,
                    'utilization': len(self.visuospatial_sketchpad) / self.capacity if self.capacity > 0 else 0
                },
                'episodic_buffer': {
                    'count': len(self.episodic_buffer),
                    'capacity': self.capacity * 2,
                    'utilization': len(self.episodic_buffer) / (self.capacity * 2) if self.capacity > 0 else 0
                }
            }
            ...
            context = {
                ...
                'high_activation_memories': high_activation_memories,
            ...
            return {
                ...
                'total_memories': len(self.memory_index) if hasattr(self, 'memory_index') else 0
            }

class BasicTextEmbeddingGenerator:
    """基本文本嵌入生成器 - 提供真实的文本嵌入功能"""
    ...
    def _initialize_basic_vectors(self):
        ...
        for i, word in enumerate(basic_words):
            ...
            for j in range(self.embedding_dim):
                # 基于词和位置的确定性函数
                vector[j] = np.sin(((zlib.adler32(str(word).encode('utf-8')) & 0xffffffff) % 1000) * 0.01 + j * 0.1) * 0.5 + 0.5
    ...
    def generate_embedding(self, text: str) -> np.ndarray:
        ...
        if not words:
            ...
            for i in range(self.embedding_dim):
                # Deterministic normal distribution using Box-Muller
                u1 = ((zlib.adler32(str(f"empty_text_{i}_u1").encode('utf-8')) & 0xffffffff) % 10000) / 10000.0
                u2 = ((zlib.adler32(str(f"empty_text_{i}_u2").encode('utf-8')) & 0xffffffff) % 10000) / 10000.0
                u1 = max(u1, 1e-10)
                u2 = max(u2, 1e-10)
                z0 = math.sqrt(-2.0 * math.log(u1)) * math.cos(2.0 * math.pi * u2)
                result[i] = z0  # standard normal
            return result
        ...
        # 初始化结果向量
        result_vector = np.zeros(self.embedding_dim)
        ...
        for word, count in word_counts.items():
            ...
            if word in self.word_vectors:
                ...
                # 简单IDF：稀有词权重更高
                idf = math.log((self.vocab_size + 1) / (1 + 1))  # 简化版本
                weight = tf * idf
                result_vector += weight * word_vector
            else:
                # 对于未知词，创建基于词本身的确定性向量
                unknown_vector = self._create_word_vector(word)
                # 未知词的权重较低
                weight = tf * 0.5
                result_vector += weight * unknown_vector
    ...
    def _create_word_vector(self, word: str) -> np.ndarray:
        ...
        for j in range(self.embedding_dim):
            # 使用词的哈希值创建确定性模式
            hash_val = (zlib.adler32(str(word).encode('utf-8')) & 0xffffffff) % 1000
            vector[j] = np.sin(hash_val * 0.01 + j * 0.1) * 0.5 + 0.5
    ...
    def _generate_basic_embedding(self, content: Any, modality: str = None) -> np.ndarray:
        """生成基本的嵌入作为最后手段"""
        # 基于内容和模态创建确定性嵌入
        ...
        if modality == 'text':
            # 对于文本，使用文本嵌入生成器
            return self.text_embedding_generator.generate_embedding(content_str)
        else:
            # 对于非文本模态，创建基于内容的确定性向量
            embedding = np.zeros(128)
            content_hash = (zlib.adler32(str(content_str).encode('utf-8')) & 0xffffffff) % 1000
            
            for i in range(128):
                embedding[i] = np.sin(content_hash * 0.01 + i * 0.1) * 0.5 + 0.5
