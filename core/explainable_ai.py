...
class DecisionTracer:
    ...
    def _generate_natural_language_explanation(self, trace):
        ...
        # 添加最终决策
        explanation_parts.append(
            f"最终决策: {trace['final_decision']} " f"(总体置信度: {trace['confidence']:.2f})"
        )
...
class ConfidenceCalibrator:
    ...
    def _calculate_average_error(self):
        ...
        errors = [
            abs(p["predicted"] - p["actual"]) for p in self.calibration_data[-50:]
        ]
    ...
    def analyze_decision(self, decision_data, features, outcome):
        ...
        for feature in features:
            ...
            if feature_value is not None:
                # 基于特征值和结果更新重要性
                importance_update = self._calculate_importance_update(
...
class CounterfactualGenerator:
    ...
    def generate_counterfactuals(self, decision_data, actual_outcome, num_scenarios=3):
        ...
        for i in range(num_scenarios):
            scenario = self._create_counterfactual_scenario(
    ...
    def _create_counterfactual_scenario(self, decision_data, actual_outcome):
        ...
        # 简化的反事实生成
        scenario_id = (
            f"counterfactual_{len(self.counterfactual_scenarios)}_{int(time.time())}"
        )
        ...
        if features:
            ...
            seed_value = (zlib.adler32(str(str(decision_data).encode('utf-8')) & 0xffffffff) + "features")
            features_to_change = sorted(
                features, key=lambda x: (zlib.adler32(str(str(x).encode('utf-8')) & 0xffffffff) + str(seed_value))
            )[:num_changes]
    ...
    def _modify_value(self, value):
        ...
        if isinstance(value, (int, float)):
            # 数值：增加或减少10-20%
            change_factor = 1.0 + (-0.2 + ((zlib.adler32(str(str(value).encode('utf-8')) & 0xffffffff) + "modify") % 41) * 0.01)
    ...
    def _analyze_scenario(self, scenario):
        ...
        analysis = {
            ...
            "outcome_difference": scenario["predicted_outcome"]
...
class ExplainableAI:
    ...
    def _generate_comprehensive_explanation(
        self, decision_process, feature_importance, counterfactuals, confidence
    ):
        """生成全面解释"""
        explanation_parts = []
        ...
        explanation_parts.append(
            f"系统可靠性: {calibration_report['reliability_score']:.3f}"
        )
    ...
    def get_system_report(self):
        ...
        return {
            ...
            "counterfactual_analysis": {
                "total_scenarios": len(
                    self.counterfactual_generator.counterfactual_scenarios
                ),
                "last_generated": datetime.fromtimestamp(
                    self.counterfactual_generator.counterfactual_scenarios[-1][
                        "timestamp"
                    ]
                ).isoformat()
                if self.counterfactual_generator.counterfactual_scenarios
                else None,
            },
