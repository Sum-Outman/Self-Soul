    ...
    def sample_batch(self, batch_size: int = 32, prioritized: bool = True):
        ...
        if prioritized and len(self.buffer) > 0:
            ...
            for i in range(min(batch_size, len(self.buffer))):
                # 确定性阈值
                threshold = ((zlib.adler32(str(str(self.buffer).encode('utf-8')) & 0xffffffff) + str(i) + str(batch_size)) % 10000) / 10000.0
                # 找到第一个累积概率大于阈值的索引
                idx = np.searchsorted(cumulative, threshold)
                # 确保索引不重复
                while idx in indices:
                    idx = (idx + 1) % len(self.buffer)
                indices.append(idx)
            ...
            # 使用确定性种子洗牌
            seed = (zlib.adler32(str(str(self.buffer).encode('utf-8')) & 0xffffffff) + str(batch_size))
            def deterministic_shuffle(arr, seed):
                ...
                return arr
            shuffled = deterministic_shuffle(indices, seed)
            indices = shuffled[:min(batch_size, len(self.buffer))]
