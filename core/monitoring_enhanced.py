...
class EnhancedSystemMonitor(SystemMonitor):
    ...
    def __new__(cls, config_path: str = "config/monitoring_config.json"):
        ...
        if cls._instance is None:
            cls._instance = super(EnhancedSystemMonitor, cls).__new__(cls)
        return cls._instance
    
    def __init__(self, config_path: str = "config/monitoring_config.json"):
        """初始化增强监控器 / Initialize enhanced monitor"""
        # 防止重复初始化
        if EnhancedSystemMonitor._initialized:
            return
            
        EnhancedSystemMonitor._initialized = True
        ...
        # 监控线程将在启动事件中启动 / Monitoring thread will be started in startup event
        self.monitoring_thread = threading.Thread(target=self._enhanced_monitoring_loop, daemon=True)
        
        self.logger.info("增强监控系统已初始化 (线程将在启动事件中启动) | Enhanced monitoring system initialized (thread will be started in startup event)")
    ...
    def _collect_collaboration_metrics(self):
        """收集协作指标 / Collect collaboration metrics - no simulated data allowed"""
        try:
            ...
            for collab_type in collaboration_types:
                if collab_type not in self.collaboration_metrics:
                    # 初始化协作指标为默认值，表示真实数据尚未收集
                    # 不使用硬编码的模拟值（如95.0%成功率，0.5秒延迟等）
                    self.collaboration_metrics[collab_type] = {
                        "success_rate": 0.0,  # 初始化为0，表示真实数据尚未收集
                        "avg_latency": 0.0,   # 初始化为0，表示真实数据尚未收集
                        "throughput": 0.0,    # 初始化为0，表示真实数据尚未收集
                        "last_updated": datetime.now(),
                        "data_source": "not_collected_yet",
                        "note": "Real collaboration metrics collection required. System needs to track actual collaboration performance."
                    }
                    
                    # 记录需要实现真实数据收集
                    self.logger.info(f"协作类型 '{collab_type}' 的指标已初始化为默认值。需要实现真实协作数据收集。")
        except Exception as e:
            ...
            if os.path.exists(log_file):
                ...
                for line in lines:
                    try:
                        ...
                        # 如果解析失败，添加原始行
                        self.logger.debug(f"日志解析失败: {log_error}")
                        logs.append({
                            "timestamp": datetime.now().isoformat(),
                            "level": "info", 
                            "message": line.strip()
